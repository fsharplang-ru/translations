...
В одном из наших предыдущих постов, "[Почему вам следует использовать F#][why-you-should-use]", перечислены несколько причин, почему уже сегодня стоит попробовать F#. В данной статье мы разберем некоторые основы, необходимые для успешного применения F#. Пост предназначен для людей с опытом C#, Java или иных объектно-ориентированных языков программирования. Понятия, представленны здесь, должны быть уже хорошо знакомы F# программистам.

В данном посте не будут предприниматься попытки показать, как вы можете "переписать" C# код на F#. Это связано с тем, что C# и F# представляют различные парадигмы, что делает каждый язык подходящим для своих собственных целей. Мы думаем, что изучение понятий функционального программирования расширит кругозор и поможет вам стать немного лучшим программистом. Вы осознаете ценность функционального программирования быстрее, если не будете думать о том, как переписать код из одной парадигмы в другую.
...
Заметим, что в этом примере нет цикла `for`. На концептуальном уровне это очень отличается от императивного кода. Мы не возводим в квадрат каждый элемент списка. С помощью функции `square` мы отображаем входной список, чтобы получить значения, возведенные в квадрат. Это очень тонкое различие, но на практике может приводить к значительно отличающемуся коду. Прежде всего, функция `getSquares` на самом деле создает полностью новый список.
