...
В одном из предыдущих постов, "[Почему вам следует использовать F#][why-you-should-use]", перечислены несколько причин, почему уже сегодня стоит попробовать F#. В данной статье разбираются некоторые основы, необходимые для успешного применения F#. Пост предназначен для людей с опытом C#, Java или иных объектно-ориентированных языков программирования. Представленные понятия должны быть уже хорошо знакомы программистам F#.

В данном посте не будут предприниматься попытки показать, как вы можете "переписать" C# код на F#. Это связано с тем, что C# и F# представляют различные парадигмы, благодаря чему каждый язык подходит для своих собственных целей. Изучение понятий функционального программирования расширит кругозор и поможет вам улучшить свои навыки программирования. Вы осознаете ценность функционального программирования быстрее, если не будете думать о том, как переписать код из одной парадигмы в другую.

А сейчас настало время любопытных, пытливых и готовых изучать совершенно новые вещи. Давайте начнем!

# Сразу к различиям

Перед тем, как мы приступим к изучению понятий функционального программирования, давайте посмотрим на небольшой пример кода на F# и определим несколько мест, в которых F# отличается от C#. Это базовый пример кода с двумя функциями и выводом результата на экран:
...
Следующая таблица показывает соответствия между некоторым ключевыми понятиями C# и F#. Это умышленно короткое и неполное описание, но так его проще запомнить в начале изучения F#.
...
Стоит отметить, что все указанное в столбце C# так же возможно в F# (и довольно легко реализуется). В столбце F# также есть вещи, которые возможны в C#, хотя их намного сложнее достигнуть. Следует упомянуть, что элементы в левом столбце не являются "плохими" в F#, и наоборот. Объекты с методами отлично подходят для использования в F# и часто являются лучшим решением для F# в зависимости от вашей ситуации.
...
Рассмотрим более конкретный пример: например, вы хотите возвести в квадрат числа из входного списка. Вот как это можно сделать в F#:
...
Заметим, что в этом примере нет цикла `for`. На концептуальном уровне это сильно отличается от императивного кода. Мы не возводим в квадрат каждый элемент списка. С помощью функции `square` мы отображаем входной список и получаем значения, возведенные в квадрат. Это очень тонкое различие, но на практике оно может приводить к значительно отличающемуся коду. Прежде всего, функция `getSquares` на самом деле создает полностью новый список.

Неизменяемость - это гораздо более широкая концепция, чем просто иной способ управления данными в списках. Понятие ссылочной прозрачности ([Referential Transparency][ref-transparency]) естественно для F#, и оказывает значительное влияние как на разработку систем, так и на то, как части этих систем сочетаются. Функциональные характеристики системы становятся более предсказумыми, когда значения не изменяются, если вы этого не ожидаете.
...
- Каждая ветка выражения `if` возвращает значение, которое в данном случае будет являться возврвщаемым значением функции `getMessage`.
...
Если копнуть немного глубже, F# на самом деле описывает инструкции с помощью выражений. Такие выражения возвращают значение типа [`unit`][unit]. `unit` немного похож на `void` в C#:
...
В предыдущем примере использования выражения `for`, всё имеет тип `unit`. Выражения типа `unit` — это выражения, которые не имеют возвращаемого значения.
...
- [Массивы в F#][arrays] — это массивы .NET. Они изменяемы - хранимые значения могут быть перезаписаны на месте. Они вычисляются энергично (eagerly).
- [Списки в F#][lists] — это неизменяемые односвязные списки. Они могут быть использованы в виде [шаблонов списков][list-patterns] для [сопоставлением с образом (pattern matching)][pattern-matching] в F#. Они вычисляются энергично.
...
# Соответствие между методами LINQ и функциями F#
...
Предудыщий раздел содержит в себе довольно много информации, но по мере написания программ на F# она станет интуитивной.

# Функциональные конвейеры

Вы могли заметить, что оператор `|>` используется в предыдущих примерах кода. Он очень похож на конвейеры в unix: принимает что-то слева от себя и передает на вход чему-то справа. Этот оператор (называется "pipe" или "pipeline") используется для создания [функциональных конвейеров][pipes]. Вот пример:
...
Поскольку F# — язык платформы .NET, в нем существуют те же примитивные типы, что и C#: `string`, `int` и так далее. Он использует объекты .NET и поддерживает четыре основных столпа объектно-ориентированного программирования. F# предоставляет [кортежи][tuples] (tuples), а также два основных типа, которые отсутствуют в C#: [записи][records] (records) и [размеченные объединения][du] (discriminated unions).

Запись — это группа упорядоченных именованных значений, которая автоматически реализует операцию сравнения - в самом буквальном смысле. Нет необходимости различать равенство ссылок или пользовательское определение равенства значения между двумя объектами. Записи — это значения, а значения можно сравнивать. Они являются типами-произведениями, если говорить на языке теории категорий. У них есть множество применений, однако одно из самых очевидных — замена POCO и POJO.
...
Другой основной тип в F# — это [размеченные объединения, или DU][du], или РО. РО — это типы, представляющие некоторое количество именованных вариантов. На языке теории категорий это называется тип-сумма. Они также могут быть определены рекурсивно, что значительно упрощает иерархические данные.
...
Наверняка вы увидели странный синтаксис в определение варианта `Node`. Это на самом деле сигнатура кортежа. Это означает, что определенное нами BST может быть или пустым, или являться кортежем `(значение, левое поддерево, правое поддерево)`. Более подробно про это написано в разделе о [сигнатурах][signatures].

# Собираем все вместе: синтаксис F# за 60 секунд

Следующий пример кода представлен с разрешения [Скотта Влашина][scott], героя сообщества F#, написавшего этот прекрасный обзор F# синтаксиса. Вы прочтете его примерно за минуту. Пример был немного отредактирован.
...
Для F# сушествует множество самоучителей, включая материалы для тех, кто пришел с опытом C# или Java. Следующие ссылки могут быть полезными по мере того, как вы будете глубже изучать F#:
...
[list-patterns]: https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/pattern-matching#list-pattern
