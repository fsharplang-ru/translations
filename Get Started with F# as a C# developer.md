...
[**Инструкции**](https://ru.wikipedia.org/wiki/%D0%9E%D0%BF%D0%B5%D1%80%D0%B0%D1%82%D0%BE%D1%80_(%D0%BF%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5)) -- это единицы работы, выполняемые императивно, путем запуска программы.

[**Выражения**](https://ru.wikipedia.org/wiki/%D0%92%D1%8B%D1%80%D0%B0%D0%B6%D0%B5%D0%BD%D0%B8%D0%B5_(%D0%B8%D0%BD%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%82%D0%B8%D0%BA%D0%B0)) -- это единицы кода, которые можно вычислить и получить значения.

[**Типы**](https://ru.wikipedia.org/wiki/%D0%A2%D0%B8%D0%BF_%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85) -- это классификация данных в программе.
...
Неизменяемость делает больше, чем просто меняет способ управления данными в списках. Понятие ссылочной прозрачности ([Referential Transparency](http://wiki.c2.com/?ReferentialTransparency)) естественно для F#, и оказывает значительное влияние как на разработку систем, так и на то, как части этих систем сочетаются. Функциональные характеристики системы становятся более предсказумыми, когда значения не изменяются, если вы этого не ожидаете.
...
- [`if...then...else`](https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/conditional-expressions-if-then-else) -- это выражение, а не инструкция.
...
Если рассмотреть немного глубже, F# на самом деле использует выражения для моделирования инструкций. Такие выражения возвращают значения типа [`unit`](https://docs.microsoft.com/ru-ru/dotnet/fsharp/language-reference/unit-type). `unit` немного похож на `void` в C#:
...
- [Массивы в F#](https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/arrays) -- это массивы .NET. Они изменяемые, что означает, что из значения могут быть изменены на месте. Они вычисляются энергично (eagerly).
- [Списки в F#](https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/lists) -- это неизменяемые односвязные списки. Они могут быть использованы в виде `list patterns` для сопоставлением с образом (pattern matching) в F#. Они вычисляются энергично.
- [Последовательности в F#](https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/sequences) является неизменяемыми `IEnumerable<T>`. Они вычисляются лениво.
...
Вы могли заметить, что `|>` используется в предыдущих примерах кода. Этот оператор очень похож на конвейеры в unix: он берет что-то слева от себя и передает на вход чему-то справа. Этот оператор (называется "pipe" или "pipeline") используется для создания [функциональных конвейеров](https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/functions/index#function-composition-and-pipelining). Вот пример:
...
Другой основной тип в F# -- это [размеченные объединения, или DU](https://docs.microsoft.com/ru-ru/dotnet/fsharp/language-reference/discriminated-unions). DU -- это типы, представляющие некоторое количество именованных вариантов. На языке теории категорий это называется тип-сумма. Они также могут быть определены рекурсивно, что значительно упрощает иерархические данные.
...
Вы можете также заметить немного занятного синтаксиса в определение варианта Node. Это на самом деле сигнатура кортежа. Это означает, что определенное нами BST, может быть или пустым, или кортежем `(значение, левое поддерево, правое поддерево). Прочитайте о [сигнатурах](https://fsharpforfunandprofit.com/posts/function-signatures/), чтобы узнать больше.
...
Есть много-много других вещей, для которых вы можете использовать F#, предыдущий список ни в коем случае не является исчерпывающим. F# используется в различных приложениях: от простых скриптов для сборки, до бэкенда многомиллионных сайтов для электронной торговли. Нет никаких ограничений по проектам, для которых вы можете использовать F#.
...
Также существуют несколько документированных способов начать изучать F#.

И наконец, F# сообществе очень дружелюбно к начинающим. Есть очень активный чат в Slack, поддерживаемый F# Software Foundation, с комнатами для начинающих, к которым вы свободно можете присоединиться. Мы настоятельно рекомендуем вам это сделать!